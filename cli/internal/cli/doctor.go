...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  yalie-pp-cli doctor
  yalie-pp-cli doctor --json
  yalie-pp-cli doctor --fail-on warn
  yalie-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set it with: echo \"$TOKEN\" | yalie-pp-cli auth set-token or export YALIE_BEARER_AUTH=\"your-token-here\""
					} else {
						authConfigured = true
			...
			if os.Getenv("YALIE_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "YALIE_BEARER_AUTH")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "YALIE_BEARER_AUTH")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "yalie-pp-cli", suggestion)
						} else {
			...
			// yalie-pp-cli bundles four independent Yale data sources under
			// one CLI (CourseTable GraphQL/REST, Canvas, Degree Audit, Yale
			// Catalog). The generic client-based checks above only cover the
			// Canvas spec client; report the other three here using each
			// source's cheapest real check.
			collectYaleSourcesReport(cmd.Context(), report)
			...
			checkKeys := []struct{ key, label string }{
				...
				{"coursetable_public", "CourseTable (public)"},
				{"coursetable_account", "CourseTable (account)"},
				{"degree_audit", "Degree Audit"},
				{"yale_catalog", "Yale Catalog"},
			}
