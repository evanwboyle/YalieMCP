...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("YALIE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("YALIE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "yalie-pp-cli/feedback")
...
When ` + "`YALIE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`YALIE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  yalie-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
  yalie-pp-cli feedback --stdin < notes.txt
  yalie-pp-cli feedback list --limit 10`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  yalie-pp-cli feedback list
  yalie-pp-cli feedback list --limit 5
  yalie-pp-cli feedback list --json`,
