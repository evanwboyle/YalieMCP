...
// yalieCookieAuthHint is appended to auth-error hints below. DEVIATION FROM
// GENERATOR TEMPLATE: this file is normally fully generated ("DO NOT EDIT"),
// but three of this CLI's four data sources (CourseTable account/worksheet
// endpoints, Degree Audit, and Canvas as a fallback) authenticate via a raw
// session cookie env var, not the spec-declared Bearer token. The generated
// hint text below only ever suggested `auth set-token` / `YALIE_BEARER_AUTH`,
// which is misleading for a 401/403 on those cookie-authenticated sources —
// confirmed during live testing against a real Yale account. Regen will
// overwrite this; if picked up as a generator issue, the fix belongs in the
// template's error-hint text generation for CLIs with multiple auth env vars.
const yalieCookieAuthHint = "\n      This CLI also supports cookie-based auth for CourseTable/Degree Audit/Canvas — " +
...
func classifyAPIErrorOnly(err error) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			yalieCookieAuthHint+
			"\n      Run 'yalie-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token."+
			"\n      Set it with: echo \"$TOKEN\" | yalie-pp-cli auth set-token or export YALIE_BEARER_AUTH=\"your-token-here\""+
			yalieCookieAuthHint+
			"\n      Run 'yalie-pp-cli doctor' to check auth status.", err))
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Set it with: echo \"$TOKEN\" | yalie-pp-cli auth set-token or export YALIE_BEARER_AUTH=\"your-token-here\""))
	}
	if len(trimmed) == 0 {
