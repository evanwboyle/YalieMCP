...
type rootFlags struct {
	...
	// invocation. Mirrors the YALIE_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "yalie-pp-cli",
		Short: `Yalie CLI — Every Yale academic tool YalieMCP had, plus offline course search, degree-audit history, and cross-source requirement…`,
		Long: `Yalie CLI — Every Yale academic tool YalieMCP had, plus offline course search, degree-audit history, and cross-source requirement matching no other Yale tool offers.
...
Health check: run 'yalie-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("yalie-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select content-type,display_name,filename)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'yalie-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCanvasCmd(flags))
	rootCmd.AddCommand(newCoursesCmd(flags))
	rootCmd.AddCommand(newSeasonsCmd(flags))
	rootCmd.AddCommand(newProfessorsCmd(flags))
	rootCmd.AddCommand(newCatalogCmd(flags))
	rootCmd.AddCommand(newCurriculumCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelCertificatesCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelMajorsCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelWorksheetCmd(flags))
