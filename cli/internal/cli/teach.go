...
// yalie-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"yalie-pp-cli/internal/learn/entities"
	"yalie-pp-cli/internal/learn/lookups"
	"yalie-pp-cli/internal/learn/patterns"
...
const noLearnEnvVar = "YALIE_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("yalie-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  yalie-pp-cli teach --query "<question>" --resource-type <type> --resource <id> --resource <id> &`,
...
when learnings exist.`,
		Example: `  yalie-pp-cli recall "<question>" --agent
  yalie-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  yalie-pp-cli learnings list --agent
  yalie-pp-cli learnings list --query "<substring>"
  yalie-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  yalie-pp-cli learnings forget "<question>" --resource <id>
  yalie-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  yalie-pp-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type "items" --entity-kind "category" --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  yalie-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
