...
type Config struct {
	...
	YalieBearerAuth  string          `toml:"bearer_auth"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://yale.instructure.com/api/v1",
	}
	...
	// Env var overrides
	if v := os.Getenv("YALIE_BEARER_AUTH"); v != "" {
		cfg.YalieBearerAuth = v
		cfg.markEnvOverride("YalieBearerAuth")
		cfg.AuthSource = "env:YALIE_BEARER_AUTH"
		cfg.CredentialSource = "env:YALIE_BEARER_AUTH"
	} else if v := os.Getenv("CANVAS_TOKEN"); v != "" {
		// Dual-mode Canvas auth, preferred branch: an admin-issued Bearer
		// token, when the student's Yale account has one. Most Yale
		// students do not (Yale blocks self-service PAT generation,
		// confirmed directly during research for this CLI) — CANVAS_COOKIE
		// below is the practical default path.
		cfg.YalieBearerAuth = v
		cfg.markEnvOverride("YalieBearerAuth")
		cfg.AuthSource = "env:CANVAS_TOKEN"
		cfg.CredentialSource = "env:CANVAS_TOKEN"
	} else if v := os.Getenv("CANVAS_COOKIE"); v != "" {
		// Dual-mode Canvas auth, fallback branch: a yale.instructure.com
		// session cookie, sent as a raw Cookie header rather than
		// Authorization — do not set YalieBearerAuth here.
		if cfg.Headers == nil {
			cfg.Headers = map[string]string{}
		}
		cfg.Headers["Cookie"] = v
		cfg.AuthSource = "env:CANVAS_COOKIE"
		cfg.CredentialSource = "env:CANVAS_COOKIE"
	}
	...
	if v := os.Getenv("YALIE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("YALIE_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "yalie-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.YalieBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:YALIE_BEARER_AUTH"
		}
		return "Bearer " + c.YalieBearerAuth
	}
	if c.AccessToken != "" {
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.YalieBearerAuth != "" {
		parts = append(parts, "bearer_auth="+c.YalieBearerAuth)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		YalieBearerAuth: c.YalieBearerAuth,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.YalieBearerAuth == "" {
		c.YalieBearerAuth = creds.YalieBearerAuth
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.YalieBearerAuth = ""
	delete(c.envOverrides, "YalieBearerAuth")
	c.updateFileConfigField("YalieBearerAuth")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["YalieBearerAuth"] {
			out.YalieBearerAuth = c.fileConfig.YalieBearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "YalieBearerAuth":
		c.fileConfig.YalieBearerAuth = c.YalieBearerAuth
	}
}
